...
class Calculator:
    ...
    @staticmethod
    def second_runge(y: List[float], h: float, p: float = 2) -> List[float]: # todo
        p = 2
        res, y2h = [], []
        for i in range(len(y)):
    ...
    def print_init(txt: str, init: List[float]):
        ...
        for i in init:
            print("{:7.4} ".format(i if i is not None else "NaN"))
    ...
    def print_res(txt: str, res: List[float]):
        ...
        for i in res:
            print("{:7} ".format(f"{i:.4f}" if i is not None else "   NaN"))
