...
int func_sqrt(table_t *table)
{
    ...
    for(int i = 0; i < table->rows; i++)
    {
        ...
        table_sqrt.table[i][2] = 1 / table->table[i][2];
    }
    for(int i = 0; i < table_sqrt.rows; i++)
    {
        for(int j = 0; j < table_sqrt.rows - i - 1; j++)
        {
            if (table_sqrt.table[j][0] > table_sqrt.table[j + 1][0])
            {
                ...
                double tmp_yy = table_sqrt.table[j][2];
                table_sqrt.table[j][2] = table_sqrt.table[j + 1][2];
                table_sqrt.table[j + 1][2] = tmp_yy;
    ...
    //print_func_table((table)*);
    double r = table->table[table->rows - 1][0];
    double l = table->table[0][0];
    double m = 0.0;
    //double y;
    //printf("%lf %lf\n", r, l);
    while (fabs(r - l) > 1e-8)
    {
        ...
        hermit.node_num = 4;
        hermit.polynom_exp = 2 * hermit.node_num - 1;
        hermit.value_to_find = m;
        int i_find = find_index_in_table(hermit.value_to_find, table);
